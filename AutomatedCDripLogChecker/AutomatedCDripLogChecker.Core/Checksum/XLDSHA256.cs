...
{
    //code from System.Security.Cryptography.SHA256Managed
    internal class XLDSHA256 : SHA256
    {
        ...
        private static uint Sigma_0l(uint x)
        {
            return (RotateRight(x, 7) ^ RotateRight(x, 18) ^ (x >> 3));
        }

        private static uint Sigma_1l(uint x)
        {
        ...
        */
        ...
        private static unsafe void SHA256Expand(uint* x)
        {
            for (int i = 16; i < 64; i++)
            {
                x[i] = Sigma_1l(x[i - 2]) + x[i - 7] + Sigma_0l(x[i - 15]) + x[i - 16];
            }
        }
